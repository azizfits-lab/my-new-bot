...
def get_stats():
    ...
    cur.execute("""
        SELECT COALESCE(token_symbol, mint) as token, COUNT(*) as c FROM transactions
        WHERE action = 'SELL' GROUP BY mint ORDER BY c DESC LIMIT 5
    """)
    top_sold = [(r["token"], r["c"]) for r in cur.fetchall()]
...
# ---------------- 🆕 Signal Events (أساس Token Score) ----------------

def log_signal_event(mint: str, event_type: str):
...
async def get_token_market_data(session: aiohttp.ClientSession, mint: str):
    ...
    result = {
        ...
        # 🆕 نسب التغيير الحقيقية (مجانية، جاهزة عند DexScreener، ماكناش كنستعملوها)
...
def build_trade_message(wallet_name, wallet_address, parsed, market_data, buyers_windows: dict,
    ...
    """
    🆕 رسالة محسّنة: كتزيد وقت العملية، عمر التوكن، عدد المشترين
    خلال 5 / 15 / 60 دقيقة الأخيرة، Token Score من 100، سطر السرد الذكي
    (Smart Narrative)، وسطر تحذير الأمان (Rug Check) إذا الخطر مازال متوسط/عالي.
    """
    ...
    if score is not None:
        score_line = f"🧠 Token Score: {score}/100 {score_label_emoji(score_label)} ({score_label})\n"

    # 🆕 سطر تحذير الأمان (Rug Check) — كيبان غير إذا الخطر متوسط/عالي (باش ما نزيدوش
    # ضجة على صفقات آمنة، ونلفتو النظر بقوة على لي فيها خطر حقيقي)
    rug_line = ""
    if rug_data is not None:
        risk_score, risk_label, warnings = calculate_rug_risk(rug_data)
        if risk_score >= 30:
            top_warning = warnings[0] if warnings else ""
            rug_line = f"🛡️ فحص الأمان: {risk_label} ({risk_score}/100)\n   {top_warning}\n"

    # 🆕 سطر/سطور السرد الذكي — غير للـ BUY، وغير إذا عندنا market_data حقيقي
    narrative_block = ""
    ...
    text = (
        ...
        f"{rug_line}"
        f"{narrative_block}\n"
        f"💼 المحفظة: <b>{wallet_name}</b>\n"
        f"📍 <code>{short_address}</code>\n"
        f"🪙 Contract: <code>{mint}</code>\n"
        f"🔢 الكمية: {amount}\n"
        f"💵 القيمة: ${usd_value:,.2f}\n"
        f"💲 السعر: ${price:.8f}\n"
        f"🧢 Market Cap: ${mcap:,.0f}\n"
        f"💧 Liquidity: ${liquidity:,.0f}\n"
        f"⏳ عمر التوكن: {token_age}\n"
        f"👥 مشترين (5د/15د/60د): {buyers_windows.get('5m', 0)} / {buyers_windows.get('15m', 0)} / {buyers_windows.get('60m', 0)}\n\n"
...
async def handle_parsed_transaction(session, bot, admin_chat_id, name, address, parsed, settings):
    ...
    try:
        mint = parsed["mint"]
        action = parsed["action"]
        is_first_buy_for_wallet = (
            action == "BUY" and not wallet_has_bought_before(address, mint)
        )
        ...
        if action == "BUY" and market_data:
            record_trade_outcome_entry(
                transaction_id, address, mint, parsed["timestamp"], market_data["price_usd"]
            )

        # 🆕 Rug Check: غير لصفقات BUY (السيلات ماعندهاش داعي نفحصو فيها الأمان)
        rug_data = None
        if action == "BUY":
        ...
        should_notify = (
            (action == "BUY" and settings["notify_buy"]) or
            (action == "SELL" and settings["notify_sell"])
        )
        if should_notify and usd_value >= settings["min_usd_alert"]:
            await broadcast(bot, admin_chat_id, message)

        # 🐋 Whale Alert
        if usd_value >= settings["whale_threshold"] and not settings["mute_whale"]:
            whale_msg = (
                f"🐋 <b>Whale Buy/Sell</b>\n\n"
                f"صفقة كبيرة بقيمة ${usd_value:,.2f}\n"
                f"💼 {name} | 🪙 {symbol}\n"
                f"<code>{mint}</code>"
            )
        ...
        if action == "BUY":
            ...
            if distinct_buyers == settings["smart_money_min"] and not was_alert_sent(mint, "smart_money") and not settings["mute_smart_money"]:
                ...
                sm_msg = (
                    ...
                    f"🪙 {symbol} — <code>{mint}</code>\n"
                    f"عدد المحافظ اللي شراوها فآخر {settings['window_minutes']} دقائق: {distinct_buyers}\n"
            ...
            if distinct_buyers == settings["multi_wallet_min"] and not was_alert_sent(mint, "multi_wallet") and not settings["mute_multi_wallet"]:
                ...
                mw_msg = (
                    ...
                    f"🪙 {symbol} — <code>{mint}</code>\n"
                    f"{distinct_buyers} محافظ شراو نفس العملة فآخر {settings['window_minutes']} دقائق!\n"
                    f"المحافظ: {', '.join(buyers)}"
                )
        ...
        if score_label in ("High Interest", "Extreme Interest") and not was_alert_sent(mint, "hot_token"):
            hot_msg = (
                ...
                f"🪙 {symbol} — <code>{mint}</code>\n"
                f"🧠 Token Score: {score}/100 {score_label_emoji(score_label)} ({score_label})\n"
                f"👥 عدد المحافظ المتابَعة لي شراتو: {distinct_buyers_ever}\n"
                f"استعمل /tokenscore {mint} لتفاصيل أكثر"
            )
...
def build_settings_text_and_keyboard():
    ...
    sell_status = "✅ مفعّل" if s.get("notify_sell") == "1" else "❌ متوقف"
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(f"Sell: {sell_status}", callback_data="toggle_sell")],
...
async def statistics(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for token, count in stats["top_bought"]:
        text += f"  • {token}: {count}\n"
    text += "\n<b>أكثر العملات بيعاً:</b>\n"
    for token, count in stats["top_sold"]:
        text += f"  • {token}: {count}\n"
...
async def wallet_stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"🟢 عدد Buy: {stats['buy_count']}\n"
        f"🔴 عدد Sell: {stats['sell_count']}\n"
        f"💵 إجمالي المشتريات: ${stats['total_buy_usd']:,.2f}\n"
        f"💰 إجمالي المبيعات: ${stats['total_sell_usd']:,.2f}\n"
...
async def run_bot_forever():
    """
    🆕 يشغل البوت (Telegram polling + Helius Webhook server إذا مفعّل) بالتوازي،
    عبر نفس الـ asyncio event loop. يبقى خدام حتى توصل إشارة إيقاف (Ctrl+C).
    """
    init_db()
    ...
    async with app:
        ...
        logger.info("🚀 البوت بدا الخدمة (النسخة المحسّنة v2 — Polling + Score + Webhooks جاهزة)...")
        try:
